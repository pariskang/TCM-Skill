...
危候判 E 级排除,而《痿论》定义性条文不被同段他句排除词误杀)+ **方证极性检测**
(NegEx/ConText 式:《伤寒论》"桂枝湯主之"处方 vs "不可與桂枝湯"禁忌,分句作用域、
按方剂,同段可肯定 A 方否定 B 方)+ **引用忠实度双层确定性核验**(evidence_span
必须逐字来自原文)。**简体查询完美支持**、语义召回离线(TF-IDF)或神经嵌入可切换,
默认全离线可跑;`eval` 命令跑**金标准回归评测**(Recall@k / MRR / 排除 / 极性 /
忠实度,17 用例,CI 可用)。当前病种 MVP:骨质疏松。
完整设计见 [`docs/GRAPHRAG.md`](docs/GRAPHRAG.md),方法学依据见 [`docs/RESEARCH.md`](docs/RESEARCH.md)。
